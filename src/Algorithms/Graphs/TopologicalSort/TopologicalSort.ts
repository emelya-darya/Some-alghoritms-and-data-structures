...
export const getTopologicalSort = function (graph: UnweightedGraphType, ignoreСyclicity = false) {
   let allGraphVertices: Set<string> | Array<string> = new Set()
   ...
   let haveNoIncomingEdges: Set<string> | Array<string> = new Set(allGraphVertices)
   for (const v in graph) {
   ...
   const topologicalSort: Record<string, number> = {}
   let time = 1
