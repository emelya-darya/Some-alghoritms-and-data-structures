...
export const getAllCyclesWithVrtx = function (withVertex: string | number, SCC: UnweightedGraphType) {
   withVertex = String(withVertex)

   const outputCycles: Array<string> = []

   const dfsStack: Array<string> = []
   const blockedSet: Set<string> = new Set()
   const blockedMap: Record<string, Array<string>> = {}

   const startIndexForNbsCycling: Record<string, number> = {}
   const partsOfTheCycles: Set<string> = new Set()
   ...
   while (dfsStack.length) {
      ...
      if (!hasUnprocessableNBs) {
         ...
         if (partsOfTheCycles.has(v)) {
            let listTmp: Set<string> = new Set(blockedMap[v] || [])
            delete blockedMap[v]
            blockedSet.delete(v)

            while (listTmp.size) {
               const nextListTmp: Set<string> = new Set()
